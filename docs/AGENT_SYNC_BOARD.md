...
| **Last updated (UTC)** | 2026-07-16T07:05Z |
| **Last updated by** | claude-fable (Fable 5, PM) |
| **▶️ START HERE (new session)** | **`docs/handoff/SESSION_HANDOFF_20260716_POST_OPUS.md`** — clean state: what is live+proven, what Opus retracted (esp. the wrong-gate replay), the ONE missing measurement (`--gates v18`, output VM `/tmp/convex_v18.log`), and the open owner decision (A: change producer → VP / B: regime awareness). Do not build on any pre-retraction claim. |
| **🧹 HANDOFF + OPUS RETRACTIONS (2026-07-16 ~05:30Z) — READ `docs/handoff/SESSION_HANDOFF_20260716_POST_OPUS.md` FIRST** | Owner instruction: start a clean session continuing from Fable, **cutting what Opus got wrong**. **RETRACTED BY OPUS ITSELF:** **(1) THE BIG ONE — the convex/exit replay tested the WRONG GATE.** It ran `--gates v17,v17-mission,none`; **`v18` IS the live config** (`_entry_gate_config` comments it *"Live V1.8 size-the-edge launcher config"*; the lane runs `DEXTER3_FABLE_VERSION=v1.8-size-the-edge` + V18_WINNER_BOOST/CHASE_RESCUE/B_TIER=1). **So every "LADDER-BASELINE (live) PF 0.38/0.44" figure below is v1.7, NOT the live system — RETRACTED.** The harness itself is sound (`_apply_entry_gate` calls the real `evaluate_v16_entry_gate`; only the preset differed). A `--gates v18` run is in flight → VM `/tmp/convex_v18.log` — **collect it first; the live baseline is currently UNKNOWN.** **(2) "Conviction is anti-predictive" — retracted** (its own 10k test: every leader_score bucket negative on validate; the live N=48 "profitable" band was the WORST). **(3) "Buy side is broken" — downgraded to hypothesis** (measured at gate=none/v17, never at v18; 7wk sample has a crash + a flat range but NO bull regime). **(4) "Cut losers faster" — retracted** (a 0.5R stop kills 37.5% of the 3R+ runners; the 1R stop kills 0.0% — DO NOT tighten). **STILL PROVEN (gate-independent):** MFE fat tail (16.2% reach ≥3R, mean 6.11R, max 20.64R) vs live capture mean +0.24R/max-ever +0.44R; trough early (p50 bar 0) & peak late (p50 bar 23, p90 bar 46); ladder givebacks 0.35-0.83 ATR = sub-bar noise → it exits on the normal dip that precedes every runner; owner's pyramid = +0.42R/runner via a limit at -0.4R (no detection needed). Live fixes all verified & unaffected (money-scaling `6a0cd3c`, smart-exit `360c43f`, twin-guard `952815a` — 6 live firings, VM==HEAD hash-checked). |
| **⚖️ (RETRACTED — wrong gate, see above) VERDICT (2026-07-16 ~03:40Z): conviction theory KILLED by its own test; the BUY SIDE is the survivor** | 10k-bar derive/validate (`--leader-buckets`, gate=none removes the 0.18 confound). **(1) CONVICTION = NOISE, theory DEAD.** The live N=48 said 0.10-0.18 was the profitable band (+13.24/WR 56.2%); the replay says that band is **derive -74.41 / validate -108.87 (WR 40.8) = the WORST validate bucket**. EVERY bucket is negative on validate ([0,0.10) -58.22, [0.18,0.25) -19.26, [0.25,0.35) -74.41, [0.35+) -38.75). High band flips sign across segments (derive +41.42 PF 1.25 → validate -38.75). **leader_score has NO stable relationship with outcome in either direction — the gate `min_leader_score=0.18` filters on a signal with no predictive content (theater, not backwards).** The N=48 live sample was noise; my anti-predictive theory is retracted. **(2) THE SURVIVOR — BUY SIDE IS STRUCTURALLY BROKEN.** Side cut: derive buy **-453.05** (WR 41.6) vs sell +331.94 (WR 51.0); validate buy **-280.49** (WR 41.1) vs sell -19.02 (WR 47.9). **REFUTATION PASS (the test that killed VP/bucket-router/PA-Eye): is it just trend?** Price drift: derive **-467.28 pts** (4496→4029, a crash — so sell's +331 there is JUST the trend, no edge) but **validate +4.83 pts = FLAT** (4029→4034 over 3 weeks). **In a FLAT market buys still lost -280.49 at WR 41.1% while sells were -19.02 — a 14.7x asymmetry regime CANNOT explain, N=2296.** This is the FIRST finding all week to survive the regime refutation. (3) `hunt_swing_structure` negative BOTH segments (-185.10/-145.08, N=2111/1432, WR 44.3/44.1) — stable loser, biggest setup by volume; `hunt_m15_drift` positive both (+40.26 WR 76.7 / +6.28 WR 60.5) but N=60/43 too small. h1_context & day_range_tilt FLIP sign across segments = regime noise. **HONEST BOTTOM LINE: the entry stream has no edge in EITHER direction — buy is actively anti-signal (41% WR), sell is coin-flip (47.9% ≈ breakeven after costs). 5th independent confirmation that decide_hunt is edge-free.** Caveat before acting: 7 weeks contain a crash + a flat range but NO sustained uptrend — the system's buys have never been observed in a bull regime, so any buy-side action must be reversible + weekly re-validated. |
| **🧠 (retracted) THE UNIFYING THEORY (2026-07-16 ~01:30Z) — CONVICTION IS ANTI-PREDICTIVE — killed by the 10k test above** | Owner pushed: "ผมยังไม่เห็นส่วนนั้นแบบลึกล้ำ... คิดใหม่ให้ลึกล้ำกว่าเดิม" — fair; every fix so far was plumbing, not edge. Joined 48 live trades (clean deals-based pnl) to their entry conviction. **The committee's own leader_score is ANTI-PREDICTIVE:** 0.10-0.18 → **net +13.24, WR 56.2%** (the ONLY profitable band); 0.18-0.25 → **-31.84, WR 35.7%**; 0.25-0.35 → -13.53, WR 33.3%; 0.35+ (elite A+) → +1.00, WR 40%. **The live gate `min_leader_score=0.18` filters OUT the profitable band and admits the losing ones — the door points backwards.** `p_win` same: higher p_win (0.47+) → WR 35.3% vs 0.45-0.47 → 46.7%. Also: **buy WR 26.1% (net -27.29) vs sell 60%**; **hunt_sweep_reclaim -28.22 @ 25% WR** (the setup that fired tonight's twins); grok lane WR 27.3% vs fable 48.6%. **THIS THEORY UNIFIES EVERY FAILED EXPERIMENT:** why 240 exit combos all lost (can't exit your way out of a backwards entry filter); why PA-Eye `support`/confirmation was ALWAYS the worst bucket; why aligned-trending = -116R; why mirror-support was positive on every gate. Mechanism = high conviction means many lenses agree means the move is already obvious means we enter late INTO the liquidity — the owner's own "เราคือ liquidity ไม่ใช่ smart money", now measured. **NOT ACTED ON: N=48 is far too small and this exact class died out-of-sample 4x (VP, bucket-router, PA-Eye, empirical sizing).** 10k-bar derive/validate diagnostic (`--leader-buckets`, report-only, gate=none to remove the 0.18 confound, + side + setup cuts) in flight. Caveats: fuzzy ts+side+setup join; align×regime cut returned empty (features absent on matched decision rows). |
