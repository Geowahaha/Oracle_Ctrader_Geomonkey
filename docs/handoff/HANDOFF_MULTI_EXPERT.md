...
- **2026-05-12 Fibo MTF planner evidence delta:** VM is on commit `67562b2` and `dexter-monitor` is active. `xau_shadow_journal` contains 541 `fibo_mtf_planner:observe_only` rows from 2026-05-11T14:58:32Z through 2026-05-12T15:36:13Z; no `probe` route sample exists yet. Hermes added `ops/fibo_mtf_planner_shadow_report.py` to summarize route/TF/zone evidence and explicitly block micro-live until Opus gates are met.
- **2026-05-12 Fibo MTF telemetry delta:** Hermes prepared shadow-only execution-anchor and MAE/MFE evidence plumbing. New Fibo MTF shadow candidates carry `execution_swing_low/high`, `local_swing_low/high`, `execution_anchor_source`, and `execution_anchor_is_live_plan=False`; shadow resolver now records `shadow_mae_rr`/`shadow_mfe_rr` alongside `shadow_pnl_rr`. This is observability only and does not enable live Fibo MTF dispatch.
- Current baton:
