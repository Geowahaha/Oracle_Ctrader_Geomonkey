...
# SNIPER lane (2026-07-30): the XAUUSD and USTEC units are two PROCESSES and
# must never share a state file, lock or log. DEXTER3_SNIPER_SUFFIX (set only
# on the USTEC unit, mirroring dexter3.sniper's label fork) suffixes all
# three paths; read at import time like the label itself.
...
def _sniper_producer_enabled() -> bool:
    """SNIPER lane (owner directive 2026-07-30): the SHADOWCODES Pine script
    translated to a producer (dexter3/sniper.py) — S/D zones + ST2 wick
    rejection + QM sweep reclaim, market entry at the M5 close, plain 2R TP.
    Forward canary on XAUUSD and USTEC (separate units via
    DEXTER3_SNIPER_SUFFIX). DEFAULT OFF; two ways in, same pattern as every
    other lane: DEXTER3_PRODUCER=sniper or DEXTER3_MODE=sniper."""
...
def _mscalp2_producer_enabled() -> bool:
    """MSCALP2 twin (owner order 2026-08-06): SAME mscalp producer/entries/
    exits, plus the fast-tick dollar-take (_run_mscalp2_take_tick) — the
    forward-A/B measuring take-fast vs bank-the-tail. DEFAULT OFF."""
    return mscalp.mscalp2_mode_enabled()


def _mscalp_brk_producer_enabled() -> bool:
    """MSCALP-BRK lane (owner observation 2026-08-07 'Buy ที่แนวต้าน Sell ที่
    แนวรับ'): mscalp signals taken ONLY in M15 blue sky — dexter3/mscalp.py's
    decide_mscalp_brk carries the evidence block. T15 exits identical to the
    mscalp original so the A/B isolates the entry. DEFAULT OFF."""
    return mscalp.mscalp_brk_mode_enabled()
...
def _active_order_label(mode: str | None = None) -> str:
    ...
    if current_mode == "mscalp-brk":
        from dexter3.mscalp import MSCALP_BRK_LABEL

        return MSCALP_BRK_LABEL
...
def _active_label_family(mode: str | None = None) -> str:
    ...
    if current_mode == "mscalp-brk":
        from dexter3.mscalp import MSCALP_BRK_LABEL_FAMILY

        return MSCALP_BRK_LABEL_FAMILY
    if current_mode == "h3fade":
...
def _apply_v16_profit_controls(
    ...
    """
    if not _env_bool("DEXTER3_V16_PROFIT_CONTROLS_ENABLED", True):
...
def _apply_v16_house_money_status(gov_state: dict[str, Any], status: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    if not _env_bool("DEXTER3_V16_HOUSE_MONEY_ENABLED", True):
        return status
    ...
    def on_decision(self, decision: hunter_brain.Decision) -> None:
        ...
        if self.manager.state.state == "FLAT":
            leg = Leg(
                ...
                label=f"{LIVE_ORDER_LABEL}:{decision.setup}",
            )
...
def run_symbol_cycle(
    ...
    *,
    ...
    for i in pending:
        ...
        if is_newest and executor is not None and lane:
        ...
        elif is_newest and _mscalp_brk_producer_enabled():
            # MSCALP-BRK lane (owner observation 2026-08-07): the same
            # impulse signal taken ONLY in M15 blue sky — losers were
            # entering 0.06xATR from the nearest level, winners had 4+xATR
            # of air. Gate evidence in dexter3/mscalp.py; blocked signals
            # journal as sr_wall skips.
            from dexter3 import market_lens as _ml

            mb_session = str(_ml.session_context(bar_ts).get("value") or "unknown")
            decision = mscalp.decide_mscalp_brk(symbol, prefix, spread_abs, session=mb_session)
            ...
            # Market entry at the M5 close, PLAIN exits (broker SL/TP; the
            # script's TP1=2R is the fixed target; 66.5% of live gross profit
            # is fixed broker TP — audit 2026-07-25). Zones are rebuilt
            # deterministically from the trailing window each decision: the
            # SAME state model in replay and live by construction.
            from dexter3 import market_lens as _ml

            sn_session = str(_ml.session_context(bar_ts).get("value") or "unknown")
            decision = sniper.decide_sniper(symbol, prefix, spread_abs, session=sn_session)
...
def _run_mscalp2_take_tick(
    state: dict[str, Any],
    symbol: str,
    executor: Dexter3Executor | None,
    lane: list[dict[str, Any]],
) -> str | None:
    """MSCALP2 dollar-take (owner order 2026-08-06 "กำไร >$X ให้ปิดเลย"):
    close the lane the first fast tick its REAL broker floating PnL reaches
    ``DEXTER3_MSCALP2_TAKE_USD``. Runs BEFORE the normal OM evaluation; the
    original bank/time/SL exits remain underneath untouched, so the twin =
    mscalp + exactly this one extra exit. Returns a status string when it
    closed the lane this tick, None to fall through. Never raises; shadow
    mode closes nothing; an unreliable PnL aggregate holds (never trade
    blind, same contract as the OM)."""
    if executor is None or not lane:
        return None
    take_usd = mscalp.mscalp2_take_usd()
    agg = basket_live.aggregate_lane(lane, base_risk_usd=_om_base_risk_usd(executor))
    if bool(agg.get("unreliable")):
        return None
    pnl = _f(agg.get("aggregate_pnl_usd"), 0.0)
    if not mscalp.mscalp2_take_decision(pnl, take_usd):
        return None
    ids = [position_id_of(p) for p in lane if position_id_of(p) > 0]
    if not ids:
        return None
    try:
        executed = executor.execute_close_all(ids, reason="usd_take")
    except Exception as exc:  # noqa: BLE001 - a failed close falls back to the OM/broker exits
        log_error(f"mscalp2_usd_take_close({symbol})", exc)
        return None
    log_line(
        f"{utc_now_iso()} {symbol} mscalp2_usd_take pnl={pnl:.2f} >= take={take_usd:.2f} "
        f"ids={ids} result={executed.get('action') if isinstance(executed, dict) else executed}"
    )
    _clear_basket_runtime(state, symbol)
...
def run_om_tick(
    ...
    *,
    ...
    # MSCALP2 dollar-take (owner order 2026-08-06): mode-gated AND env-gated
    # (DEXTER3_MSCALP2_TAKE_USD > 0, default 0=off) — every other lane, and
    # mscalp2 itself with the env absent, falls through unchanged.
    if _mscalp2_producer_enabled() and mscalp.mscalp2_take_usd() > 0:
...
def run_loop(symbols: list[str], poll_sec: int, live: bool = False) -> None:
    ...
    # SNIPER canary lane (2026-07-30): own broker label per symbol-unit
    # (suffix-forked family) so no other lane — nor the sibling sniper
    # unit on the other symbol — ever touches its positions.
    if is_sniper:
    ...
    if is_mscalp_brk:
        from dexter3.mscalp import MSCALP_BRK_LABEL as _MSK_LABEL

        import dexter3.executor as _ex
        _ex.LABEL = _MSK_LABEL
        print(f"[MSCALP-BRK] Forced executor LABEL to {_MSK_LABEL}", flush=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if os.environ.get("DEXTER3_MODE", "v16").lower().strip() == "sniper":
        from dexter3.sniper import SNIPER_LABEL as _SN_LABEL
    ...
    if os.environ.get("DEXTER3_MODE", "v16").lower().strip() == "mscalp2":
        from dexter3.mscalp import MSCALP2_LABEL as _MS2_LABEL
    ...
    if os.environ.get("DEXTER3_MODE", "v16").lower().strip() == "mscalp-brk":
        from dexter3.mscalp import MSCALP_BRK_LABEL as _MSK_LABEL
